...
class Root(DeclNode):
    ...
    def __str__(self) -> str:
        ...
        for i in range(len(self.decls)):
            res += str(self.specs[i]) + ',' + str(self.decls[i])

        return f'BEGINROOT,{res},ENDROOT,'
...
class Pointer(DeclNode):
    ...
    def __str__(self) -> str:
        if self.const:
            return f'CONST POINTER {self.child}'
        # print('pointer childs : ', self.child, type(self.child))
        return f'POINTER,{self.child}'
...
class Array(DeclNode):
    ...
    def __str__(self) -> str:
        return f'ARRAY,{self.child}[{self.n}]'
...
class Function(DeclNode):
    ...
    def __str__(self):
        ...
        for i in range(len(self.args)):
            sep = ', ' if i < (len(self.args) - 1) else ''
            res += str(self.args[i]) + sep
            # print('Fn ARGS: ', str(self.args[i]), type(str(self.args[i])))

        return f'FUNCTION,BEGINFUNCPARAMS{res},ENDFUNCPARAMS,STARTFUNCBODY,{self.child},ENDFUNCBODY'
...
class Identifier(DeclNode):
    ...
    def __str__(self) -> str:
        return f'IDENTIFIER,{str(self.identifier)}' if self.identifier else ''
