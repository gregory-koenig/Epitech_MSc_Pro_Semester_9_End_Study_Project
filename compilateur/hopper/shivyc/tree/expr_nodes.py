...
class String(_LExprNode):
    ...
    def __str__(self) -> str:
        return f'STRING,{repr("".join([chr(el) for el in self.chars[0:-1]]))}'
...
class ParenExpr(nodes.Node):
    ...
    def __str__(self) -> str:
        return f'PARENTEXPR,{str(self.expr)}'
...
class Div(_ArithBinOp):
    ...
    def __str__(self) -> str:
        return f'DIV,{self.left}{self.op}{self.right}'
...
class LessThanOrEq(_Relational):
    ...
    def __str__(self) -> str:
        # print("ici", type(self.left), self.right, self.op)
        return f'LESSTHENOREQ,{self.left}{self.op}{self.right}'
...
class _BoolAndOr(_RExprNode):
    ...
    def __str__(self) -> str:
        op = self.op if str(self.op) != '||' else 'OR'
...
class Equals(_RExprNode):
    ...
    def __str__(self) -> str:
        return f'EQUALS,{self.left}{self.op}{self.right}'
...
class SizeofExpr(_SizeofNode):
    ...
    def __str__(self) -> str:
        return f'SIZEOFEXPR,{str(self.expr)}'
...
class SizeofType(_SizeofNode, Declaration):
    ...
    def __str__(self) -> str:
        # print('SIZEOF : ', self.node, self.body)
        return f'SIZEOF,{self.node}'
...
class ArraySubsc(_LExprNode):
    ...
    def __str__(self) -> str:
        return f'ARRAYSUBSCRIPTION,{self.head}[{self.arg}],'
...
class FuncCall(_RExprNode):
    ...
    def __str__(self) -> str:
        ...
        return f'FUNCCALL,{self.func}({args})'
